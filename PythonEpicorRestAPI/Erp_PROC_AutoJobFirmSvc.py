...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.AutoJobFirmSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.AutoJobFirmSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.AutoJobFirmSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_SubmitToAgent(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.PROC.AutoJobFirmSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
# Custom Schemas:
#########################################################################
class Erp_Tablesets_AutoJobFirmRow:
   def __init__(self, obj):
      self.FromStartDate:str = obj["FromStartDate"]
      """  From Job Start Date  """  
      self.Company:str = obj["Company"]
      self.LogFile:str = obj["LogFile"]
      """  Log File  """  
      self.PartList:str = obj["PartList"]
      self.PlannerList:str = obj["PlannerList"]
      self.PlantList:str = obj["PlantList"]
      self.ProdGrupList:str = obj["ProdGrupList"]
      self.SelectForMassPrint:bool = obj["SelectForMassPrint"]
      """  Select jobs marked as released to be printed during a mass print of the job traveler  """  
      self.ToStartDate:str = obj["ToStartDate"]
      """  To Job Start Date  """  
      self.WithinMLT:bool = obj["WithinMLT"]
      """  Auto select within Manufacturing Lead Time for each assembly: Scheduled Start Date <= Today + MLT  """  
      self.AsmblPAA:bool = obj["AsmblPAA"]
      """  Consider Plan as Assembly Demand Subassemblies  """  
      self.LogLevel:str = obj["LogLevel"]
      """  Logging Level: Process\Detail  """  
      self.AsmblFromStartDate:str = obj["AsmblFromStartDate"]
      """  Assembly From Start Date  """  
      self.AsmblToStartDate:str = obj["AsmblToStartDate"]
      self.AsmblFromStartDateToken:str = obj["AsmblFromStartDateToken"]
      self.AsmblToStartDateToken:str = obj["AsmblToStartDateToken"]
      self.FromStartDateToken:str = obj["FromStartDateToken"]
      self.ToStartDateToken:str = obj["ToStartDateToken"]
      self.LogPathFile:str = obj["LogPathFile"]
      """  Log Path File  """  
...
class Erp_Tablesets_AutoJobFirmTableset:
   def __init__(self, obj):
      self.AutoJobFirm:list[Erp_Tablesets_AutoJobFirmRow] = obj["AutoJobFirm"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_AutoJobFirmTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_AutoJobFirmTableset] = obj["returnObj"]
      pass
