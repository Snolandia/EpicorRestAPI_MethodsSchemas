...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.MYLMWFinishedGoodsSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.MYLMWFinishedGoodsSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def post_SetFiscalDates(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.RPT.MYLMWFinishedGoodsSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_CheckBaseCurrency(epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.RPT.MYLMWFinishedGoodsSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.MYLMWFinishedGoodsSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
# Custom Schemas:
#########################################################################
class CheckBaseCurrency_output:
   def __init__(self, obj):
      self.returnObj:str = obj["returnObj"]
      """  warning message  """  
      pass

class Erp_Tablesets_MYLMWFinishedGoodsParamRow:
   def __init__(self, obj):
      ...
      """  The Part List  """  
      self.SelectByPeriod:bool = obj["SelectByPeriod"]
      self.IsIncludeAll:bool = obj["IsIncludeAll"]
      """  Include parts without movements  """  
      self.PartType:str = obj["PartType"]
      self.AnnualLMWYear:int = obj["AnnualLMWYear"]
      """   Report Year for "Annual LMW M4 Report"
This is considered if ReportMode = 1  """  
      self.ReportMode:int = obj["ReportMode"]
...
class Erp_Tablesets_MYLMWFinishedGoodsTableset:
   def __init__(self, obj):
      self.MYLMWFinishedGoodsParam:list[Erp_Tablesets_MYLMWFinishedGoodsParamRow] = obj["MYLMWFinishedGoodsParam"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_MYLMWFinishedGoodsTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_MYLMWFinishedGoodsTableset] = obj["returnObj"]
      pass
