import asyncio
import aiohttp
import configEpicorSchemas



# Title: Erp.BO.WHTPaymentSubmitSvc
# Description: Withholding Tax Payment Submit Report BO
# Version: v1



#########################################################################
# OData methods:
#########################################################################
async def getServiceDocument(epicorHeaders = None):
   """  
   Summary: Get service document
   Description: Get service document for the service
   OperationID: GetServiceDocument
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => application/json
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/",headers=creds) as resp:
           return await resp.json()

async def get_metadata(epicorHeaders = None):
   """  
   Summary: Get metadata document
   Description: Get service ODATA metadata in XML format
   OperationID: GetMetadata
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: Returns metadata document => content
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/$metadata",headers=creds) as resp:
           return await resp.json()

async def get_WHTPaymentSubmits(select = None, expand = None, filter = None, orderby = None, top = None, skip = None, inlinecount = None, epicorHeaders = None):
   """  
   Summary: Calls GetRows for the service
   Description: Get WHTPaymentSubmits items from the server using GetRows standard method. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li></ul></div>
   OperationID: GetRows_WHTPaymentSubmits
      :param select: Desc: Odata select comma delimited list of fields
      :param expand: Desc: Odata expand to child
      :param filter: Desc: Odata filter results
      :param orderby: Desc: Odata sort results
      :param top: Desc: Odata top results
      :param skip: Desc: Odata skip results
      :param inlinecount: Desc: Odata.count value
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/Epicor.RESTApi.Help.ODataSetResponse_System.Collections.Generic.List_Erp.Tablesets.WHTPaymentSubmitRow
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmits",headers=creds) as resp:
           return await resp.json()

async def post_WHTPaymentSubmits(requestBody, epicorHeaders = None):
   """  
   Summary: Calls UpdateExt to create new item for the service
   Description: Calls UpdateExt to create new item for the service. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li></ul></div>
   OperationID: NewUpdateExt_WHTPaymentSubmits
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/Erp.Tablesets.WHTPaymentSubmitRow
   Returns: 
      201 Desc: Resource is created. Operation is successful.  => reference#/components/schemas/Erp.Tablesets.WHTPaymentSubmitRow
      400 Desc: Unable to deserialize entity. Input data is not in correct format.
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmits", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def get_WHTPaymentSubmits_Company_SubmitReportID(Company, SubmitReportID, select = None, expand = None, filter = None, epicorHeaders = None):
   """  
   Summary: Calls GetByID to retrieve the WHTPaymentSubmit item
   Description: Calls GetByID to retrieve the WHTPaymentSubmit item by specified keys. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li><li>String parameters should be specified in single quotes</li></ul></div>
   OperationID: GetById_WHTPaymentSubmit
      :param Company: Desc: Company   Required: True   Allow empty value : True
      :param SubmitReportID: Desc: SubmitReportID   Required: True   Allow empty value : True
      :param select: Desc: Odata select comma delimited list of fields
      :param expand: Desc: Odata expand to child
      :param filter: Desc: Odata filter results
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/Erp.Tablesets.WHTPaymentSubmitRow
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmits(" + Company + "," + SubmitReportID + ")",headers=creds) as resp:
           return await resp.json()

async def patch_WHTPaymentSubmits_Company_SubmitReportID(Company, SubmitReportID, requestBody, epicorHeaders = None):
   """  
   Summary: Calls UpdateExt to update WHTPaymentSubmit for the service
   Description: Calls UpdateExt to update WHTPaymentSubmit. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li><li>String parameters should be specified in single quotes</li></ul></div>
   OperationID: UpdateExt_WHTPaymentSubmit
      :param Company: Desc: Company   Required: True   Allow empty value : True
      :param SubmitReportID: Desc: SubmitReportID   Required: True   Allow empty value : True
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: input params  => reference#/components/schemas/Erp.Tablesets.WHTPaymentSubmitRow
   Returns: 
      204 Desc: No Content. Operation is successful.
      400 Desc: Unable to deserialize entity. Input data is not in correct format.
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.patch(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmits(" + Company + "," + SubmitReportID + ")", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def delete_WHTPaymentSubmits_Company_SubmitReportID(Company, SubmitReportID, epicorHeaders = None):
   """  
   Summary: Call UpdateExt to delete WHTPaymentSubmit item
   Description: Call UpdateExt to delete WHTPaymentSubmit item. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li><li>String parameters should be specified in single quotes</li></ul></div>
   OperationID: DeleteUpdateExt_WHTPaymentSubmit
      :param Company: Desc: Company   Required: True   Allow empty value : True
      :param SubmitReportID: Desc: SubmitReportID   Required: True   Allow empty value : True
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      204 Desc: No Content. Operation is successful.
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.delete(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmits(" + Company + "," + SubmitReportID + ")",headers=creds) as resp:
           return await resp.json()

async def get_WHTPaymentSubmits_Company_SubmitReportID_WHTPaymentSubmitAttches(Company, SubmitReportID, select = None, filter = None, orderby = None, top = None, skip = None, inlinecount = None, epicorHeaders = None):
   """  
   Summary: Calls GetByID for the service
   Description: Get WHTPaymentSubmitAttches items from the server using GetByID standard method. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li><li>String parameters should be specified in single quotes</li></ul></div>
   OperationID: GetByID_WHTPaymentSubmitAttches1
      :param Company: Desc: Company   Required: True   Allow empty value : True
      :param SubmitReportID: Desc: SubmitReportID   Required: True   Allow empty value : True
      :param select: Desc: Odata select comma delimited list of fields
      :param filter: Desc: Odata filter results
      :param orderby: Desc: Odata sort results
      :param top: Desc: Odata top results
      :param skip: Desc: Odata skip results
      :param inlinecount: Desc: Odata.count value
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/Epicor.RESTApi.Help.ODataSetResponse_System.Collections.Generic.List_Erp.Tablesets.WHTPaymentSubmitAttchRow
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmits(" + Company + "," + SubmitReportID + ")/WHTPaymentSubmitAttches",headers=creds) as resp:
           return await resp.json()

async def get_WHTPaymentSubmits_Company_SubmitReportID_WHTPaymentSubmitAttches_Company_SubmitReportID_DrawingSeq(Company, SubmitReportID, DrawingSeq, select = None, filter = None, epicorHeaders = None):
   """  
   Summary: Calls GetByID to retrieve the WHTPaymentSubmitAttch item
   Description: Calls GetByID to retrieve the WHTPaymentSubmitAttch item by specified keys. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li><li>String parameters should be specified in single quotes</li></ul></div>
   OperationID: GetById_WHTPaymentSubmitAttch1
      :param Company: Desc: Company   Required: True   Allow empty value : True
      :param SubmitReportID: Desc: SubmitReportID   Required: True   Allow empty value : True
      :param DrawingSeq: Desc: DrawingSeq   Required: True
      :param select: Desc: Odata select comma delimited list of fields
      :param filter: Desc: Odata filter results
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/Erp.Tablesets.WHTPaymentSubmitAttchRow
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmits(" + Company + "," + SubmitReportID + ")/WHTPaymentSubmitAttches(" + Company + "," + SubmitReportID + "," + DrawingSeq + ")",headers=creds) as resp:
           return await resp.json()

async def get_WHTPaymentSubmitAttches(select = None, filter = None, orderby = None, top = None, skip = None, inlinecount = None, epicorHeaders = None):
   """  
   Summary: Calls GetRows for the service
   Description: Get WHTPaymentSubmitAttches items from the server using GetRows standard method. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li></ul></div>
   OperationID: GetRows_WHTPaymentSubmitAttches
      :param select: Desc: Odata select comma delimited list of fields
      :param filter: Desc: Odata filter results
      :param orderby: Desc: Odata sort results
      :param top: Desc: Odata top results
      :param skip: Desc: Odata skip results
      :param inlinecount: Desc: Odata.count value
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/Epicor.RESTApi.Help.ODataSetResponse_System.Collections.Generic.List_Erp.Tablesets.WHTPaymentSubmitAttchRow
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmitAttches",headers=creds) as resp:
           return await resp.json()

async def post_WHTPaymentSubmitAttches(requestBody, epicorHeaders = None):
   """  
   Summary: Calls UpdateExt to create new item for the service
   Description: Calls UpdateExt to create new item for the service. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li></ul></div>
   OperationID: NewUpdateExt_WHTPaymentSubmitAttches
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/Erp.Tablesets.WHTPaymentSubmitAttchRow
   Returns: 
      201 Desc: Resource is created. Operation is successful.  => reference#/components/schemas/Erp.Tablesets.WHTPaymentSubmitAttchRow
      400 Desc: Unable to deserialize entity. Input data is not in correct format.
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmitAttches", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def get_WHTPaymentSubmitAttches_Company_SubmitReportID_DrawingSeq(Company, SubmitReportID, DrawingSeq, select = None, filter = None, epicorHeaders = None):
   """  
   Summary: Calls GetByID to retrieve the WHTPaymentSubmitAttch item
   Description: Calls GetByID to retrieve the WHTPaymentSubmitAttch item by specified keys. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li><li>String parameters should be specified in single quotes</li></ul></div>
   OperationID: GetById_WHTPaymentSubmitAttch
      :param Company: Desc: Company   Required: True   Allow empty value : True
      :param SubmitReportID: Desc: SubmitReportID   Required: True   Allow empty value : True
      :param DrawingSeq: Desc: DrawingSeq   Required: True
      :param select: Desc: Odata select comma delimited list of fields
      :param filter: Desc: Odata filter results
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/Erp.Tablesets.WHTPaymentSubmitAttchRow
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmitAttches(" + Company + "," + SubmitReportID + "," + DrawingSeq + ")",headers=creds) as resp:
           return await resp.json()

async def patch_WHTPaymentSubmitAttches_Company_SubmitReportID_DrawingSeq(Company, SubmitReportID, DrawingSeq, requestBody, epicorHeaders = None):
   """  
   Summary: Calls UpdateExt to update WHTPaymentSubmitAttch for the service
   Description: Calls UpdateExt to update WHTPaymentSubmitAttch. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li><li>String parameters should be specified in single quotes</li></ul></div>
   OperationID: UpdateExt_WHTPaymentSubmitAttch
      :param Company: Desc: Company   Required: True   Allow empty value : True
      :param SubmitReportID: Desc: SubmitReportID   Required: True   Allow empty value : True
      :param DrawingSeq: Desc: DrawingSeq   Required: True
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: input params  => reference#/components/schemas/Erp.Tablesets.WHTPaymentSubmitAttchRow
   Returns: 
      204 Desc: No Content. Operation is successful.
      400 Desc: Unable to deserialize entity. Input data is not in correct format.
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.patch(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmitAttches(" + Company + "," + SubmitReportID + "," + DrawingSeq + ")", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def delete_WHTPaymentSubmitAttches_Company_SubmitReportID_DrawingSeq(Company, SubmitReportID, DrawingSeq, epicorHeaders = None):
   """  
   Summary: Call UpdateExt to delete WHTPaymentSubmitAttch item
   Description: Call UpdateExt to delete WHTPaymentSubmitAttch item. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li><li>String parameters should be specified in single quotes</li></ul></div>
   OperationID: DeleteUpdateExt_WHTPaymentSubmitAttch
      :param Company: Desc: Company   Required: True   Allow empty value : True
      :param SubmitReportID: Desc: SubmitReportID   Required: True   Allow empty value : True
      :param DrawingSeq: Desc: DrawingSeq   Required: True
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      204 Desc: No Content. Operation is successful.
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.delete(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/WHTPaymentSubmitAttches(" + Company + "," + SubmitReportID + "," + DrawingSeq + ")",headers=creds) as resp:
           return await resp.json()

async def get_List(select = None, filter = None, orderby = None, top = None, skip = None, inlinecount = None, epicorHeaders = None):
   """  
   Summary: Calls GetList for the service
   Description: Get list of items<div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li></ul></div>
   OperationID: GetList
      :param select: Desc: Odata select comma delimited list of fields
      :param filter: Desc: Odata filter results
      :param orderby: Desc: Odata sort results
      :param top: Desc: Odata top results
      :param skip: Desc: Odata skip results
      :param inlinecount: Desc: Odata.count value
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/Epicor.RESTApi.Help.ODataSetResponse_System.Collections.Generic.List_Erp.Tablesets.WHTPaymentSubmitListRow
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List",headers=creds) as resp:
           return await resp.json()




#########################################################################
# Custom methods:
#########################################################################
async def get_GetRows(whereClauseWHTPaymentSubmit, whereClauseWHTPaymentSubmitAttch, pageSize, absolutePage, epicorHeaders = None):
   """  
   Summary: Invoke method GetRows
   Description: Returns a dataset containing all rows that satisfy the where clauses.
   OperationID: Get_GetRows
   Required: True   Allow empty value : True
   Required: True   Allow empty value : True
   Required: True
   Required: True
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/GetRows_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  

   firstParam = True
   params = ""
   if(firstParam):
      params += "?"
      firstParam = False
   else:
      params += "&"
   params += "whereClauseWHTPaymentSubmit=" + whereClauseWHTPaymentSubmit
   if(firstParam):
      params += "?"
      firstParam = False
   else:
      params += "&"
   params += "whereClauseWHTPaymentSubmitAttch=" + whereClauseWHTPaymentSubmitAttch
   if(firstParam):
      params += "?"
      firstParam = False
   else:
      params += "&"
   params += "pageSize=" + pageSize
   if(firstParam):
      params += "?"
      firstParam = False
   else:
      params += "&"
   params += "absolutePage=" + absolutePage

   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List" + params,headers=creds) as resp:
           return await resp.json()

async def get_GetByID(submitReportID, epicorHeaders = None):
   """  
   Summary: Invoke method GetByID
   Description: Returns a DataSet given the primary key.
   OperationID: Get_GetByID
   Required: True   Allow empty value : True
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/GetByID_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  

   firstParam = True
   params = ""
   if(firstParam):
      params += "?"
      firstParam = False
   else:
      params += "&"
   params += "submitReportID=" + submitReportID

   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List" + params,headers=creds) as resp:
           return await resp.json()

async def get_GetList(whereClause, pageSize, absolutePage, epicorHeaders = None):
   """  
   Summary: Invoke method GetList
   Description: Returns a list of rows that satisfy the where clause.
   OperationID: Get_GetList
      :param whereClause: Desc: An expression used to filter the rows. Can be left blank for all rows.   Required: True   Allow empty value : True
      :param pageSize: Desc: The maximum number of rows to return. Leave as zero for no maximum.   Required: True
      :param absolutePage: Desc: Page of rows to return.   Required: True
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/GetList_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  

   firstParam = True
   params = ""
   if(firstParam):
      params += "?"
      firstParam = False
   else:
      params += "&"
   params += "whereClause=" + whereClause
   if(firstParam):
      params += "?"
      firstParam = False
   else:
      params += "&"
   params += "pageSize=" + pageSize
   if(firstParam):
      params += "?"
      firstParam = False
   else:
      params += "&"
   params += "absolutePage=" + absolutePage

   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List" + params,headers=creds) as resp:
           return await resp.json()

async def post_SetDatePeriod(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method SetDatePeriod
   Description: Call this method to set date period.
   OperationID: SetDatePeriod
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/SetDatePeriod_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/SetDatePeriod_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_GetWHTaxReports(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method GetWHTaxReports
   Description: This method get Withholding Tax Reports for specific period
   OperationID: GetWHTaxReports
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/GetWHTaxReports_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/GetWHTaxReports_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_ExcludeWHTaxReport(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method ExcludeWHTaxReport
   Description: This method exclude Withholding Tax Report from current export
   OperationID: ExcludeWHTaxReport
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/ExcludeWHTaxReport_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/ExcludeWHTaxReport_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_GenExportFile(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method GenExportFile
   Description: Export Withholding Tax Payments to file.
   OperationID: GenExportFile
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/GenExportFile_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/GenExportFile_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_SetSubmitted(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method SetSubmitted
   Description: This method set status to Submitted
   OperationID: SetSubmitted
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/SetSubmitted_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/SetSubmitted_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_ResetStatus(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method ResetStatus
   Description: This method reset status from Submitted
   OperationID: ResetStatus
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/ResetStatus_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/ResetStatus_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_PostReport(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method PostReport
   Description: This method set status to Closed
   OperationID: PostReport
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/PostReport_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/PostReport_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_GetNewWHTPaymentSubmit(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method GetNewWHTPaymentSubmit
   Description: Inserts a new row in the DataSet with defaults populated.
   OperationID: GetNewWHTPaymentSubmit
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/GetNewWHTPaymentSubmit_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/GetNewWHTPaymentSubmit_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_GetNewWHTPaymentSubmitAttch(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method GetNewWHTPaymentSubmitAttch
   Description: Inserts a new row in the DataSet with defaults populated.
   OperationID: GetNewWHTPaymentSubmitAttch
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/GetNewWHTPaymentSubmitAttch_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/GetNewWHTPaymentSubmitAttch_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_DeleteByID(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method DeleteByID
   Description: Deletes a row given its ID.
   OperationID: DeleteByID
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/DeleteByID_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/DeleteByID_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def get_GetBySysRowID(id, epicorHeaders = None):
   """  
   Summary: Invoke method GetBySysRowID
   OperationID: Get_GetBySysRowID
   Required: True   Allow empty value : True
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/GetBySysRowID_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  

   firstParam = True
   params = ""
   if(firstParam):
      params += "?"
      firstParam = False
   else:
      params += "&"
   params += "id=" + id

   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List" + params,headers=creds) as resp:
           return await resp.json()

async def get_GetBySysRowIDs(ids, epicorHeaders = None):
   """  
   Summary: Invoke method GetBySysRowIDs
   OperationID: Get_GetBySysRowIDs
   Required: True
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
   Returns: 
      200 Desc: OK => reference#/components/schemas/GetBySysRowIDs_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  

   firstParam = True
   params = ""
   if(firstParam):
      params += "?"
      firstParam = False
   else:
      params += "&"
   params += "ids=" + ids

   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List" + params,headers=creds) as resp:
           return await resp.json()

async def post_Update(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method Update
   Description: Commits the DataSet changes to the data store.
   OperationID: Update
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/Update_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/Update_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_UpdateExt(requestBody, epicorHeaders = None):
   """  
   Summary: Invoke method UpdateExt
   Description: Apply input data to service by calling GetByID/GetNew/Update methods.
   OperationID: UpdateExt
      :param epicorHeaders: A string representing the epicor log in information to be used, 
         already converted to base64 in the format username:password, defaults to the configEpicorSchemas creds
      :param requestBody: Desc: Input parameters  => reference#/components/schemas/UpdateExt_input
   Returns: 
      200 Desc: OK => reference#/components/schemas/UpdateExt_output
      500 Desc: Internal server error. Server is unable to process the request.
   """  


   creds = configEpicorSchemas.epicorCreds
   if(epicorHeaders != None):
         creds = epicorHeaders

   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WHTPaymentSubmitSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()




#########################################################################
# OData Schemas:
#########################################################################
class Epicor_RESTApi_Help_ODataSetResponse_System_Collections_Generic_List_Erp_Tablesets_WHTPaymentSubmitAttchRow:
   def __init__(self, obj):
      self.odatametadata:str = obj["odatametadata"]
      self.value:list[Erp_Tablesets_WHTPaymentSubmitAttchRow] = obj["value"]
      pass

class Epicor_RESTApi_Help_ODataSetResponse_System_Collections_Generic_List_Erp_Tablesets_WHTPaymentSubmitListRow:
   def __init__(self, obj):
      self.odatametadata:str = obj["odatametadata"]
      self.value:list[Erp_Tablesets_WHTPaymentSubmitListRow] = obj["value"]
      pass

class Epicor_RESTApi_Help_ODataSetResponse_System_Collections_Generic_List_Erp_Tablesets_WHTPaymentSubmitRow:
   def __init__(self, obj):
      self.odatametadata:str = obj["odatametadata"]
      self.value:list[Erp_Tablesets_WHTPaymentSubmitRow] = obj["value"]
      pass

class Erp_Tablesets_WHTPaymentSubmitAttchRow:
   def __init__(self, obj):
      self.Company:str = obj["Company"]
      self.SubmitReportID:str = obj["SubmitReportID"]
      self.DrawingSeq:int = obj["DrawingSeq"]
      self.XFileRefNum:int = obj["XFileRefNum"]
      self.SysRevID:int = obj["SysRevID"]
      self.SysRowID:str = obj["SysRowID"]
      self.ForeignSysRowID:str = obj["ForeignSysRowID"]
      self.DrawDesc:str = obj["DrawDesc"]
      self.FileName:str = obj["FileName"]
      self.PDMDocID:str = obj["PDMDocID"]
      self.DocTypeID:str = obj["DocTypeID"]
      self.RowMod:str = obj["RowMod"]
      """  RowMod  """  
      pass

class Erp_Tablesets_WHTPaymentSubmitListRow:
   def __init__(self, obj):
      self.Company:str = obj["Company"]
      """  Company Identifier.  """  
      self.SubmitReportID:str = obj["SubmitReportID"]
      """  Submit Report ID.  """  
      self.Description:str = obj["Description"]
      """  Report Description.  """  
      self.Module:str = obj["Module"]
      """  Module AR or AP.  """  
      self.ReportDate:str = obj["ReportDate"]
      """  Report date.  """  
      self.FiscalCalendarID:str = obj["FiscalCalendarID"]
      """  The unique identifier of the fiscal calendar.  """  
      self.FiscalYear:int = obj["FiscalYear"]
      """  Fiscal year.  """  
      self.FiscalYearSuffix:str = obj["FiscalYearSuffix"]
      """  Fiscal year suffix.  """  
      self.Quarter:str = obj["Quarter"]
      """  Quarter of fiscal year.  """  
      self.RptCatCode:str = obj["RptCatCode"]
      """  Tax Report Category.  """  
      self.StartDate:str = obj["StartDate"]
      """  Start date of the report period.  """  
      self.EndDate:str = obj["EndDate"]
      """  End date of the report period.  """  
      self.SubmittedBy:str = obj["SubmittedBy"]
      """  The user who submitted the report.  """  
      self.SubmittedOn:str = obj["SubmittedOn"]
      """  The date and time of submitting.  """  
      self.ReportStatus:int = obj["ReportStatus"]
      """  Report Status (0 - Created; 1 - Ready; 2 - Submitted; 3 - Closed)  """  
      self.ReportSequence:int = obj["ReportSequence"]
      """  The unique report sequence.  """  
      self.EFTHeadUID:int = obj["EFTHeadUID"]
      """  Electronic Interface Unique Identifier.  """  
      self.OutputFile:str = obj["OutputFile"]
      """  Output file path.  """  
      self.TestMode:bool = obj["TestMode"]
      """  Export in test mode.  """  
      self.CreatedBy:str = obj["CreatedBy"]
      """  The user who created the report.  """  
      self.CreatedOn:str = obj["CreatedOn"]
      """  The date and time of creating.  """  
      self.ChangedBy:str = obj["ChangedBy"]
      """  The last user who changed the report.  """  
      self.ChangedOn:str = obj["ChangedOn"]
      """  The date and time of last changing.  """  
      self.SysRowID:str = obj["SysRowID"]
      """  SysRowID  """  
      self.RowMod:str = obj["RowMod"]
      """  RowMod  """  
      pass

class Erp_Tablesets_WHTPaymentSubmitRow:
   def __init__(self, obj):
      self.Company:str = obj["Company"]
      """  Company Identifier.  """  
      self.SubmitReportID:str = obj["SubmitReportID"]
      """  Submit Report ID.  """  
      self.Description:str = obj["Description"]
      """  Report Description.  """  
      self.Module:str = obj["Module"]
      """  Module AR or AP.  """  
      self.ReportDate:str = obj["ReportDate"]
      """  Report date.  """  
      self.FiscalCalendarID:str = obj["FiscalCalendarID"]
      """  The unique identifier of the fiscal calendar.  """  
      self.FiscalYear:int = obj["FiscalYear"]
      """  Fiscal year.  """  
      self.FiscalYearSuffix:str = obj["FiscalYearSuffix"]
      """  Fiscal year suffix.  """  
      self.Quarter:str = obj["Quarter"]
      """  Quarter of fiscal year.  """  
      self.RptCatCode:str = obj["RptCatCode"]
      """  Tax Report Category.  """  
      self.StartDate:str = obj["StartDate"]
      """  Start date of the report period.  """  
      self.EndDate:str = obj["EndDate"]
      """  End date of the report period.  """  
      self.SubmittedBy:str = obj["SubmittedBy"]
      """  The user who submitted the report.  """  
      self.SubmittedOn:str = obj["SubmittedOn"]
      """  The date and time of submitting.  """  
      self.ReportStatus:int = obj["ReportStatus"]
      """  Report Status (0 - Created; 1 - Ready; 2 - Submitted; 3 - Closed)  """  
      self.Notes:str = obj["Notes"]
      """  Report comment.  """  
      self.ReportSequence:int = obj["ReportSequence"]
      """  The unique report sequence.  """  
      self.EFTHeadUID:int = obj["EFTHeadUID"]
      """  Electronic Interface Unique Identifier.  """  
      self.OutputFile:str = obj["OutputFile"]
      """  Output file path.  """  
      self.TestMode:bool = obj["TestMode"]
      """  Export in test mode.  """  
      self.TokenForPreviousQuarter:str = obj["TokenForPreviousQuarter"]
      """  Token number for previous period.  """  
      self.CreatedBy:str = obj["CreatedBy"]
      """  The user who created the report.  """  
      self.CreatedOn:str = obj["CreatedOn"]
      """  The date and time of creating.  """  
      self.ChangedBy:str = obj["ChangedBy"]
      """  The last user who changed the report.  """  
      self.ChangedOn:str = obj["ChangedOn"]
      """  The date and time of last changing.  """  
      self.SysRevID:int = obj["SysRevID"]
      """  SysRevID  """  
      self.SysRowID:str = obj["SysRowID"]
      """  SysRowID  """  
      self.Period:int = obj["Period"]
      """  Period  """  
      self.DateSelection:str = obj["DateSelection"]
      """  DateSelection  """  
      self.FiscalCalendarDescription:str = obj["FiscalCalendarDescription"]
      """  Fiscal calendar description  """  
      self.IsClosed:bool = obj["IsClosed"]
      """  Report is closed for changing or not.  """  
      self.IsSubmitted:bool = obj["IsSubmitted"]
      """  Report was submitted or not.  """  
      self.HasWHTPaymentReports:bool = obj["HasWHTPaymentReports"]
      self.BitFlag:int = obj["BitFlag"]
      self.RowMod:str = obj["RowMod"]
      """  RowMod  """  
      pass




#########################################################################
# Custom Schemas:
#########################################################################
class DeleteByID_input:
   """ Required : 
   submitReportID
   """  
   def __init__(self, obj):
      self.submitReportID:str = obj["submitReportID"]
      pass

class DeleteByID_output:
   def __init__(self, obj):
      pass

class Erp_Tablesets_UpdExtWHTPaymentSubmitTableset:
   def __init__(self, obj):
      self.WHTPaymentSubmit:list[Erp_Tablesets_WHTPaymentSubmitRow] = obj["WHTPaymentSubmit"]
      self.WHTPaymentSubmitAttch:list[Erp_Tablesets_WHTPaymentSubmitAttchRow] = obj["WHTPaymentSubmitAttch"]
      self.ExtensionTables:list[Ice_Extensions_ExtensionTableData] = obj["ExtensionTables"]
      pass

class Erp_Tablesets_WHTPaymentSubmitAttchRow:
   def __init__(self, obj):
      self.Company:str = obj["Company"]
      self.SubmitReportID:str = obj["SubmitReportID"]
      self.DrawingSeq:int = obj["DrawingSeq"]
      self.XFileRefNum:int = obj["XFileRefNum"]
      self.SysRevID:int = obj["SysRevID"]
      self.SysRowID:str = obj["SysRowID"]
      self.ForeignSysRowID:str = obj["ForeignSysRowID"]
      self.DrawDesc:str = obj["DrawDesc"]
      self.FileName:str = obj["FileName"]
      self.PDMDocID:str = obj["PDMDocID"]
      self.DocTypeID:str = obj["DocTypeID"]
      self.RowMod:str = obj["RowMod"]
      """  RowMod  """  
      pass

class Erp_Tablesets_WHTPaymentSubmitListRow:
   def __init__(self, obj):
      self.Company:str = obj["Company"]
      """  Company Identifier.  """  
      self.SubmitReportID:str = obj["SubmitReportID"]
      """  Submit Report ID.  """  
      self.Description:str = obj["Description"]
      """  Report Description.  """  
      self.Module:str = obj["Module"]
      """  Module AR or AP.  """  
      self.ReportDate:str = obj["ReportDate"]
      """  Report date.  """  
      self.FiscalCalendarID:str = obj["FiscalCalendarID"]
      """  The unique identifier of the fiscal calendar.  """  
      self.FiscalYear:int = obj["FiscalYear"]
      """  Fiscal year.  """  
      self.FiscalYearSuffix:str = obj["FiscalYearSuffix"]
      """  Fiscal year suffix.  """  
      self.Quarter:str = obj["Quarter"]
      """  Quarter of fiscal year.  """  
      self.RptCatCode:str = obj["RptCatCode"]
      """  Tax Report Category.  """  
      self.StartDate:str = obj["StartDate"]
      """  Start date of the report period.  """  
      self.EndDate:str = obj["EndDate"]
      """  End date of the report period.  """  
      self.SubmittedBy:str = obj["SubmittedBy"]
      """  The user who submitted the report.  """  
      self.SubmittedOn:str = obj["SubmittedOn"]
      """  The date and time of submitting.  """  
      self.ReportStatus:int = obj["ReportStatus"]
      """  Report Status (0 - Created; 1 - Ready; 2 - Submitted; 3 - Closed)  """  
      self.ReportSequence:int = obj["ReportSequence"]
      """  The unique report sequence.  """  
      self.EFTHeadUID:int = obj["EFTHeadUID"]
      """  Electronic Interface Unique Identifier.  """  
      self.OutputFile:str = obj["OutputFile"]
      """  Output file path.  """  
      self.TestMode:bool = obj["TestMode"]
      """  Export in test mode.  """  
      self.CreatedBy:str = obj["CreatedBy"]
      """  The user who created the report.  """  
      self.CreatedOn:str = obj["CreatedOn"]
      """  The date and time of creating.  """  
      self.ChangedBy:str = obj["ChangedBy"]
      """  The last user who changed the report.  """  
      self.ChangedOn:str = obj["ChangedOn"]
      """  The date and time of last changing.  """  
      self.SysRowID:str = obj["SysRowID"]
      """  SysRowID  """  
      self.RowMod:str = obj["RowMod"]
      """  RowMod  """  
      pass

class Erp_Tablesets_WHTPaymentSubmitListTableset:
   def __init__(self, obj):
      self.WHTPaymentSubmitList:list[Erp_Tablesets_WHTPaymentSubmitListRow] = obj["WHTPaymentSubmitList"]
      self.ExtensionTables:list[Ice_Extensions_ExtensionTableData] = obj["ExtensionTables"]
      pass

class Erp_Tablesets_WHTPaymentSubmitRow:
   def __init__(self, obj):
      self.Company:str = obj["Company"]
      """  Company Identifier.  """  
      self.SubmitReportID:str = obj["SubmitReportID"]
      """  Submit Report ID.  """  
      self.Description:str = obj["Description"]
      """  Report Description.  """  
      self.Module:str = obj["Module"]
      """  Module AR or AP.  """  
      self.ReportDate:str = obj["ReportDate"]
      """  Report date.  """  
      self.FiscalCalendarID:str = obj["FiscalCalendarID"]
      """  The unique identifier of the fiscal calendar.  """  
      self.FiscalYear:int = obj["FiscalYear"]
      """  Fiscal year.  """  
      self.FiscalYearSuffix:str = obj["FiscalYearSuffix"]
      """  Fiscal year suffix.  """  
      self.Quarter:str = obj["Quarter"]
      """  Quarter of fiscal year.  """  
      self.RptCatCode:str = obj["RptCatCode"]
      """  Tax Report Category.  """  
      self.StartDate:str = obj["StartDate"]
      """  Start date of the report period.  """  
      self.EndDate:str = obj["EndDate"]
      """  End date of the report period.  """  
      self.SubmittedBy:str = obj["SubmittedBy"]
      """  The user who submitted the report.  """  
      self.SubmittedOn:str = obj["SubmittedOn"]
      """  The date and time of submitting.  """  
      self.ReportStatus:int = obj["ReportStatus"]
      """  Report Status (0 - Created; 1 - Ready; 2 - Submitted; 3 - Closed)  """  
      self.Notes:str = obj["Notes"]
      """  Report comment.  """  
      self.ReportSequence:int = obj["ReportSequence"]
      """  The unique report sequence.  """  
      self.EFTHeadUID:int = obj["EFTHeadUID"]
      """  Electronic Interface Unique Identifier.  """  
      self.OutputFile:str = obj["OutputFile"]
      """  Output file path.  """  
      self.TestMode:bool = obj["TestMode"]
      """  Export in test mode.  """  
      self.TokenForPreviousQuarter:str = obj["TokenForPreviousQuarter"]
      """  Token number for previous period.  """  
      self.CreatedBy:str = obj["CreatedBy"]
      """  The user who created the report.  """  
      self.CreatedOn:str = obj["CreatedOn"]
      """  The date and time of creating.  """  
      self.ChangedBy:str = obj["ChangedBy"]
      """  The last user who changed the report.  """  
      self.ChangedOn:str = obj["ChangedOn"]
      """  The date and time of last changing.  """  
      self.SysRevID:int = obj["SysRevID"]
      """  SysRevID  """  
      self.SysRowID:str = obj["SysRowID"]
      """  SysRowID  """  
      self.Period:int = obj["Period"]
      """  Period  """  
      self.DateSelection:str = obj["DateSelection"]
      """  DateSelection  """  
      self.FiscalCalendarDescription:str = obj["FiscalCalendarDescription"]
      """  Fiscal calendar description  """  
      self.IsClosed:bool = obj["IsClosed"]
      """  Report is closed for changing or not.  """  
      self.IsSubmitted:bool = obj["IsSubmitted"]
      """  Report was submitted or not.  """  
      self.HasWHTPaymentReports:bool = obj["HasWHTPaymentReports"]
      self.BitFlag:int = obj["BitFlag"]
      self.RowMod:str = obj["RowMod"]
      """  RowMod  """  
      pass

class Erp_Tablesets_WHTPaymentSubmitTableset:
   def __init__(self, obj):
      self.WHTPaymentSubmit:list[Erp_Tablesets_WHTPaymentSubmitRow] = obj["WHTPaymentSubmit"]
      self.WHTPaymentSubmitAttch:list[Erp_Tablesets_WHTPaymentSubmitAttchRow] = obj["WHTPaymentSubmitAttch"]
      self.ExtensionTables:list[Ice_Extensions_ExtensionTableData] = obj["ExtensionTables"]
      pass

class ExcludeWHTaxReport_input:
   """ Required : 
   reportID
   submitReportID
   """  
   def __init__(self, obj):
      self.reportID:str = obj["reportID"]
      """  WHT Report ID  """  
      self.submitReportID:str = obj["submitReportID"]
      """  WHT Payment Submit Report ID  """  
      pass

class ExcludeWHTaxReport_output:
   def __init__(self, obj):
      self.returnObj:bool = obj["returnObj"]
      pass

class GenExportFile_input:
   """ Required : 
   submitReportID
   eftHeadUID
   exportFileName
   """  
   def __init__(self, obj):
      self.submitReportID:str = obj["submitReportID"]
      """  WHT Payment Submit Report ID  """  
      self.eftHeadUID:int = obj["eftHeadUID"]
      """  Electronic Interface ID  """  
      self.exportFileName:str = obj["exportFileName"]
      """  Export File Name  """  
      pass

class GenExportFile_output:
   def __init__(self, obj):
      self.returnObj:str = obj["returnObj"]
      """  Generated File Name  """  
      pass

   def parameters(self, obj):
      self.opMessage:str = obj["parameters"]
      pass

      """  output parameters  """  

class GetByID_input:
   """ Required : 
   submitReportID
   """  
   def __init__(self, obj):
      self.submitReportID:str = obj["submitReportID"]
      pass

class GetByID_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["returnObj"]
      pass

class GetBySysRowID_input:
   """ Required : 
   id
   """  
   def __init__(self, obj):
      self.id:str = obj["id"]
      pass

class GetBySysRowID_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["returnObj"]
      pass

class GetBySysRowIDs_input:
   """ Required : 
   ids
   """  
   def __init__(self, obj):
      self.ids:str = obj["ids"]
      pass

class GetBySysRowIDs_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["returnObj"]
      pass

class GetList_input:
   """ Required : 
   whereClause
   pageSize
   absolutePage
   """  
   def __init__(self, obj):
      self.whereClause:str = obj["whereClause"]
      """  An expression used to filter the rows. Can be left blank for all rows.  """  
      self.pageSize:int = obj["pageSize"]
      """  The maximum number of rows to return. Leave as zero for no maximum.  """  
      self.absolutePage:int = obj["absolutePage"]
      """  Page of rows to return.  """  
      pass

class GetList_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_WHTPaymentSubmitListTableset] = obj["returnObj"]
      pass

   def parameters(self, obj):
      self.morePages:bool = obj["morePages"]
      pass

      """  output parameters  """  

class GetNewWHTPaymentSubmitAttch_input:
   """ Required : 
   ds
   submitReportID
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["ds"]
      self.submitReportID:str = obj["submitReportID"]
      pass

class GetNewWHTPaymentSubmitAttch_output:
   def __init__(self, obj):
      pass

   def parameters(self, obj):
      self.ds:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["ds"]
      pass

      """  output parameters  """  

class GetNewWHTPaymentSubmit_input:
   """ Required : 
   ds
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["ds"]
      pass

class GetNewWHTPaymentSubmit_output:
   def __init__(self, obj):
      pass

   def parameters(self, obj):
      self.ds:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["ds"]
      pass

      """  output parameters  """  

class GetRows_input:
   """ Required : 
   whereClauseWHTPaymentSubmit
   whereClauseWHTPaymentSubmitAttch
   pageSize
   absolutePage
   """  
   def __init__(self, obj):
      self.whereClauseWHTPaymentSubmit:str = obj["whereClauseWHTPaymentSubmit"]
      self.whereClauseWHTPaymentSubmitAttch:str = obj["whereClauseWHTPaymentSubmitAttch"]
      self.pageSize:int = obj["pageSize"]
      self.absolutePage:int = obj["absolutePage"]
      pass

class GetRows_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["returnObj"]
      pass

   def parameters(self, obj):
      self.morePages:bool = obj["morePages"]
      pass

      """  output parameters  """  

class GetWHTaxReports_input:
   """ Required : 
   submitReportID
   """  
   def __init__(self, obj):
      self.submitReportID:str = obj["submitReportID"]
      """  WHT Report ID  """  
      pass

class GetWHTaxReports_output:
   def __init__(self, obj):
      self.returnObj:bool = obj["returnObj"]
      pass

class Ice_BOUpdErrorRow:
   def __init__(self, obj):
      self.TableName:str = obj["TableName"]
      self.ErrorLevel:str = obj["ErrorLevel"]
      self.ErrorType:str = obj["ErrorType"]
      self.ErrorText:str = obj["ErrorText"]
      self.ErrorSysRowID:str = obj["ErrorSysRowID"]
      self.SysRowID:str = obj["SysRowID"]
      self.RowMod:str = obj["RowMod"]
      pass

class Ice_BOUpdErrorTableset:
   def __init__(self, obj):
      self.BOUpdError:list[Ice_BOUpdErrorRow] = obj["BOUpdError"]
      self.ExtensionTables:list[Ice_Extensions_ExtensionTableData] = obj["ExtensionTables"]
      pass

class Ice_Extensions_ExtensionRow:
   def __init__(self, obj):
      self.ColumnValues:object
      self.RowMod:str = obj["RowMod"]
      self.SysRowID:str = obj["SysRowID"]
      pass

class Ice_Extensions_ExtensionTableColumn:
   def __init__(self, obj):
      self.ColumnName:str = obj["ColumnName"]
      self.ColumnType:str = obj["ColumnType"]
      pass

class Ice_Extensions_ExtensionTableData:
   def __init__(self, obj):
      self.Table:list[Ice_Extensions_ExtensionRow] = obj["Table"]
      self.SystemCode:str = obj["SystemCode"]
      self.TableName:str = obj["TableName"]
      self.Columns:list[Ice_Extensions_ExtensionTableColumn] = obj["Columns"]
      self.PrimaryKeyColumns:str = obj["PrimaryKeyColumns"]
      self.PeerTableSystemCode:str = obj["PeerTableSystemCode"]
      self.PeerTableName:str = obj["PeerTableName"]
      pass

class PostReport_input:
   """ Required : 
   submitReportID
   """  
   def __init__(self, obj):
      self.submitReportID:str = obj["submitReportID"]
      """  WHT Payment Submit Report ID  """  
      pass

class PostReport_output:
   def __init__(self, obj):
      pass

class ResetStatus_input:
   """ Required : 
   submitReportID
   """  
   def __init__(self, obj):
      self.submitReportID:str = obj["submitReportID"]
      """  WHT Payment Submit Report ID  """  
      pass

class ResetStatus_output:
   def __init__(self, obj):
      pass

class SetDatePeriod_input:
   """ Required : 
   ds
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["ds"]
      pass

class SetDatePeriod_output:
   def __init__(self, obj):
      pass

   def parameters(self, obj):
      self.ds:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["ds"]
      pass

      """  output parameters  """  

class SetSubmitted_input:
   """ Required : 
   submitReportID
   """  
   def __init__(self, obj):
      self.submitReportID:str = obj["submitReportID"]
      """  WHT Payment Submit Report ID  """  
      pass

class SetSubmitted_output:
   def __init__(self, obj):
      pass

class UpdateExt_input:
   """ Required : 
   ds
   continueProcessingOnError
   rollbackParentOnChildError
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_UpdExtWHTPaymentSubmitTableset] = obj["ds"]
      self.continueProcessingOnError:bool = obj["continueProcessingOnError"]
      self.rollbackParentOnChildError:bool = obj["rollbackParentOnChildError"]
      pass

class UpdateExt_output:
   def __init__(self, obj):
      self.returnObj:list[Ice_BOUpdErrorTableset] = obj["returnObj"]
      pass

   def parameters(self, obj):
      self.ds:list[Erp_Tablesets_UpdExtWHTPaymentSubmitTableset] = obj["ds"]
      self.errorsOccurred:bool = obj["errorsOccurred"]
      pass

      """  output parameters  """  

class Update_input:
   """ Required : 
   ds
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["ds"]
      pass

class Update_output:
   def __init__(self, obj):
      pass

   def parameters(self, obj):
      self.ds:list[Erp_Tablesets_WHTPaymentSubmitTableset] = obj["ds"]
      pass

      """  output parameters  """  

