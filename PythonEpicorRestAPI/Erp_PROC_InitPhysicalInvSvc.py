...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.InitPhysicalInvSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.InitPhysicalInvSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.InitPhysicalInvSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_SubmitToAgent(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.PROC.InitPhysicalInvSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
# Custom Schemas:
#########################################################################
class Erp_Tablesets_InitPhysicalInvParamRow:
   def __init__(self, obj):
      ...
      self.IncludeZeroQOH:bool = obj["IncludeZeroQOH"]
      ...
      self.EnableExcludePCID:bool = obj["EnableExcludePCID"]
...
class Erp_Tablesets_InitPhysicalInvTableset:
   def __init__(self, obj):
      self.InitPhysicalInvParam:list[Erp_Tablesets_InitPhysicalInvParamRow] = obj["InitPhysicalInvParam"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_InitPhysicalInvTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_InitPhysicalInvTableset] = obj["returnObj"]
      pass
