...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Ice.PROC.DataFabricProcessorSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Ice.PROC.DataFabricProcessorSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Ice.PROC.DataFabricProcessorSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_SubmitToAgent(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Ice.PROC.DataFabricProcessorSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
class GetDefaults_input:
   ...
   def __init__(self, obj):
      self.ds:list[Ice_Tablesets_DataFabricProcessorTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Ice_Tablesets_DataFabricProcessorTableset] = obj["returnObj"]
      pass
...
class Ice_Tablesets_DataFabricProcessorParamRow:
   def __init__(self, obj):
      self.CompactedKeyList:str = obj["CompactedKeyList"]
      """  Compacted Keys being processed by this worker.  """  
      self.EventTypeList:str = obj["EventTypeList"]
      """  Event Types to be processed by this worker.  """  
      self.ParentSysTaskNum:int = obj["ParentSysTaskNum"]
      """  Task that owns, and started this child worker.  """  
      self.RunDuration:int = obj["RunDuration"]
      """  Duration the processor should run for, in seconds. This is NOT used by workers.  """  
      self.SingletonEventTypeList:str = obj["SingletonEventTypeList"]
      """  Singleton Event Types to be processed by this worker.  """  
      self.WorkerID:str = obj["WorkerID"]
...
class Ice_Tablesets_DataFabricProcessorTableset:
   def __init__(self, obj):
      self.DataFabricProcessorParam:list[Ice_Tablesets_DataFabricProcessorParamRow] = obj["DataFabricProcessorParam"]
