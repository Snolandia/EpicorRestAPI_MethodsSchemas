...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.LoadCostWBDetailsSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.LoadCostWBDetailsSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
from the Group.
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.PROC.LoadCostWBDetailsSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.LoadCostWBDetailsSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
# Custom Schemas:
#########################################################################
class Erp_Tablesets_LoadCostWBDetailsRow:
   def __init__(self, obj):
      ...
      self.GetBurdenRates:bool = obj["GetBurdenRates"]
      self.GetLaborRates:bool = obj["GetLaborRates"]
      ...
      self.GetUnlinkedPurchaseParts:bool = obj["GetUnlinkedPurchaseParts"]
      self.GetUnlinkedRates:bool = obj["GetUnlinkedRates"]
      self.LogFileName:str = obj["LogFileName"]
      self.GetKitParts:bool = obj["GetKitParts"]
...
class Erp_Tablesets_LoadCostWBDetailsTableset:
   def __init__(self, obj):
      self.LoadCostWBDetails:list[Erp_Tablesets_LoadCostWBDetailsRow] = obj["LoadCostWBDetails"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_LoadCostWBDetailsTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_LoadCostWBDetailsTableset] = obj["returnObj"]
      pass
