...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.CVXA0081Svc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.CVXA0081Svc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.CVXA0081Svc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_SubmitToAgent(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.PROC.CVXA0081Svc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
# Custom Schemas:
#########################################################################
class Erp_Tablesets_CVXA0081ParamRow:
...
class Erp_Tablesets_CVXA0081Tableset:
   def __init__(self, obj):
      self.CVXA0081Param:list[Erp_Tablesets_CVXA0081ParamRow] = obj["CVXA0081Param"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_CVXA0081Tableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_CVXA0081Tableset] = obj["returnObj"]
      pass
