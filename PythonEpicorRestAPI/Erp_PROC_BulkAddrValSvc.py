...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.BulkAddrValSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.BulkAddrValSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.BulkAddrValSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_SubmitToAgent(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.PROC.BulkAddrValSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
# Custom Schemas:
#########################################################################
class Erp_Tablesets_BulkAddrValParamRow:
   def __init__(self, obj):
      self.AllCountry:bool = obj["AllCountry"]
      """  All Country filter for the report  """  
      self.AllName:bool = obj["AllName"]
      """  All Name filter for the report  """  
      self.AllZip:bool = obj["AllZip"]
      """  All Zip filter for the report  """  
      self.CountryFrom:str = obj["CountryFrom"]
      """  Country From value for report filter  """  
      self.CountryTo:str = obj["CountryTo"]
      """  Country Through value for report filter  """  
      self.IncludeValidatedAddr:bool = obj["IncludeValidatedAddr"]
      """  Flag to indicate whether to include addresses already validated  """  
      self.NameFrom:str = obj["NameFrom"]
      """  Name From value for report filter  """  
      self.NameTo:str = obj["NameTo"]
...
class Erp_Tablesets_BulkAddrValTableset:
   def __init__(self, obj):
      self.BulkAddrValParam:list[Erp_Tablesets_BulkAddrValParamRow] = obj["BulkAddrValParam"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_BulkAddrValTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_BulkAddrValTableset] = obj["returnObj"]
      pass
