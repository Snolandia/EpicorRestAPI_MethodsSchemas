...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.AssetOverviewSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.AssetOverviewSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
Notes:
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.RPT.AssetOverviewSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
Notes:   Get the default Asset Register
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.RPT.AssetOverviewSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.AssetOverviewSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
# Custom Schemas:
#########################################################################
class Erp_Tablesets_AssetOverviewParamRow:
   def __init__(self, obj):
      ...
      self.OnlyPostedDepr:bool = obj["OnlyPostedDepr"]
      """  Display only Posted Depreciations  """  
      self.ShowAdditions:bool = obj["ShowAdditions"]
      """  Show additions for the Asset  """  
      self.ShowCurrentCosts:bool = obj["ShowCurrentCosts"]
      """  Show current costs for the asset  """  
      self.ShowDisposals:bool = obj["ShowDisposals"]
      """  Show Disposals for asset  """  
      self.ShowImpairments:bool = obj["ShowImpairments"]
      """  Show impairments for the Asset  """  
      self.AssetRegID:str = obj["AssetRegID"]
      """  Asset Register ID  """  
      self.ShowUnpostedDepr:bool = obj["ShowUnpostedDepr"]
      """  Show Unposted Depreciations  """  
      self.ShowDeprDtl:bool = obj["ShowDeprDtl"]
      """  Show Depreciation Details  """  
      self.ShowAssetPerPage:bool = obj["ShowAssetPerPage"]
      """  Show one asset per page  """  
      self.ShowRevaluations:bool = obj["ShowRevaluations"]
      """  Show Revaluations for the asset  """  
      self.PrintSelCriteria:bool = obj["PrintSelCriteria"]
      self.COOneTimeIDList:str = obj["COOneTimeIDList"]
...
class Erp_Tablesets_AssetOverviewTableset:
   def __init__(self, obj):
      self.AssetOverviewParam:list[Erp_Tablesets_AssetOverviewParamRow] = obj["AssetOverviewParam"]
...
class GetApplicationDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_AssetOverviewTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_AssetOverviewTableset] = obj["returnObj"]
      pass
