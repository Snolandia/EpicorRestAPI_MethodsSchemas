...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.ContLandedCostSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.ContLandedCostSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
Notes: Returns a list of Sort options.
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.RPT.ContLandedCostSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def post_OnSupplierChange(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.RPT.ContLandedCostSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.ContLandedCostSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
# Custom Schemas:
#########################################################################
class Erp_Tablesets_ContLandedCostParamRow:
   def __init__(self, obj):
      self.Arrived:bool = obj["Arrived"]
      self.ContainerLineRef:str = obj["ContainerLineRef"]
      self.Duties:bool = obj["Duties"]
      self.Imported:bool = obj["Imported"]
      self.IndirectCosts:bool = obj["IndirectCosts"]
      self.Ordered:bool = obj["Ordered"]
      self.Received:bool = obj["Received"]
      self.Shipments:str = obj["Shipments"]
      self.Shipped:bool = obj["Shipped"]
      self.SortBy:str = obj["SortBy"]
      self.Uplift:bool = obj["Uplift"]
...
class Erp_Tablesets_ContLandedCostTableset:
   def __init__(self, obj):
      self.ContLandedCostParam:list[Erp_Tablesets_ContLandedCostParamRow] = obj["ContLandedCostParam"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_ContLandedCostTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_ContLandedCostTableset] = obj["returnObj"]
      pass
...
class OnSupplierChange_output:
   ...
   def parameters(self, obj):
      self.opSupplierNum:int = obj["parameters"]
      self.opSupplierName:str = obj["parameters"]
      pass
