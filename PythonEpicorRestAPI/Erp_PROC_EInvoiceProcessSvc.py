...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.EInvoiceProcessSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.EInvoiceProcessSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
# Custom methods:
#########################################################################
async def post_GetNewParametersExt(requestBody, epicorHeaders = None):
   """  
...
Note: A parameter dataset should never contain more than one record.
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.PROC.EInvoiceProcessSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_GetNewParametersForReceipt(requestBody, epicorHeaders = None):
   """  
...
async def post_ProcessInvoice(requestBody, epicorHeaders = None):
   """  
...
async def post_ProcessReceipt(requestBody, epicorHeaders = None):
   """  
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.EInvoiceProcessSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
# Custom Schemas:
#########################################################################
class Erp_Tablesets_EInvoiceProcessParamRow:
   def __init__(self, obj):
      ...
      self.InvoiceFrom:str = obj["InvoiceFrom"]
      """  Begin Invoice  """  
      self.InvoiceTo:str = obj["InvoiceTo"]
      """  End Invoice  """  
      self.SearchByOption:str = obj["SearchByOption"]
      """   N - Invoice Number            
L - Legal Number  """  
      self.SkipSendEmail:bool = obj["SkipSendEmail"]
      """  Skip sending email if true  """  
      self.StatusError:bool = obj["StatusError"]
      """  Error Status - 1  """  
      self.StatusGenerated:bool = obj["StatusGenerated"]
      """  Generated Status - 2  """  
      self.StatusOpen:bool = obj["StatusOpen"]
      self.StatusSent:bool = obj["StatusSent"]
      """  Sent Status - 3  """  
      self.TranDocType:str = obj["TranDocType"]
      self.TranDocTypeDescription:str = obj["TranDocTypeDescription"]
      self.TranDocTypeOption:str = obj["TranDocTypeOption"]
      """   A - All invoices and credit memos
S - Just selected  """  
      self.TrackerMode:bool = obj["TrackerMode"]
      self.TerminalIP:str = obj["TerminalIP"]
      """  User terminal IP  """  
      self.TerminalName:str = obj["TerminalName"]
      """  User terminal Name  """  
      self.CustIDList:str = obj["CustIDList"]
      """  List of selected customer IDs to process  """  
      self.StatusDelivered:bool = obj["StatusDelivered"]
      """  Delivered Status - 5  """  
      self.ReceiptGroupID:str = obj["ReceiptGroupID"]
      self.ReceiptHeadNum:int = obj["ReceiptHeadNum"]
      self.ReceiptTranDocTypeID:str = obj["ReceiptTranDocTypeID"]
      self.ReceiptTranDocTypeOption:str = obj["ReceiptTranDocTypeOption"]
...
class Erp_Tablesets_EInvoiceProcessTableset:
   def __init__(self, obj):
      self.EInvoiceProcessParam:list[Erp_Tablesets_EInvoiceProcessParamRow] = obj["EInvoiceProcessParam"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_EInvoiceProcessTableset] = obj["ds"]
      pass
...
class GetNewParametersExt_input:
   """ Required : 
   invoiceNum
   """  
   def __init__(self, obj):
      self.invoiceNum      #schema had no properties on an object.
      pass

class GetNewParametersExt_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_EInvoiceProcessTableset] = obj["returnObj"]
      pass

class GetNewParametersForReceipt_input:
   """ Required : 
   groupID
   headNum
   """  
...
class GetNewParametersForReceipt_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_EInvoiceProcessTableset] = obj["returnObj"]
      pass
...
class ProcessInvoice_input:
   ...
   def __init__(self, obj):
      ...
      self.reportStyleNum:int = obj["reportStyleNum"]
      self.useSSRSRenderFormat:str = obj["useSSRSRenderFormat"]
      pass

class ProcessInvoice_output:
   def __init__(self, obj):
      self.returnObj:int = obj["returnObj"]
      pass

class ProcessReceipt_input:
   """ Required : 
   groupID
   headNum
   checkRef
