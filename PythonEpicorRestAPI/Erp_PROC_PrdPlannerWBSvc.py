...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.PrdPlannerWBSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.PrdPlannerWBSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.PrdPlannerWBSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_SubmitToAgent(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.PROC.PrdPlannerWBSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
# Custom Schemas:
#########################################################################
class Erp_Tablesets_PrdPlannerWBParamRow:
   def __init__(self, obj):
      self.CutOffDate:str = obj["CutOffDate"]
      self.Parts:str = obj["Parts"]
      self.Jobs:str = obj["Jobs"]
      self.ProdGroups:str = obj["ProdGroups"]
      self.CurrPlantOnly:bool = obj["CurrPlantOnly"]
      self.CutOffDateToken:str = obj["CutOffDateToken"]
      self.IgnoreBackflush:bool = obj["IgnoreBackflush"]
      self.MtlHorizon:int = obj["MtlHorizon"]
      self.IncludeReleased:bool = obj["IncludeReleased"]
      """  Include Released Jobs  """  
      self.IgnoreExpPO:bool = obj["IgnoreExpPO"]
      self.IgnorePastUnfirm:bool = obj["IgnorePastUnfirm"]
      self.IgnoreNonStock:bool = obj["IgnoreNonStock"]
      self.IgnoreFutureRec:bool = obj["IgnoreFutureRec"]
      self.Contracts:str = obj["Contracts"]
      """  Filter - Planning Contract  """  
      self.IgnorePCLinks:bool = obj["IgnorePCLinks"]
...
class Erp_Tablesets_PrdPlannerWBTableset:
   def __init__(self, obj):
      self.PrdPlannerWBParam:list[Erp_Tablesets_PrdPlannerWBParamRow] = obj["PrdPlannerWBParam"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_PrdPlannerWBTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_PrdPlannerWBTableset] = obj["returnObj"]
      pass
