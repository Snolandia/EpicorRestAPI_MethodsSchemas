...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.PositivePaySvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.PositivePaySvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def post_ChangeBankAcctID(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.PROC.PositivePaySvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_ChangeEFTHeadUID(requestBody, epicorHeaders = None):
   """  
...
async def post_GetActualDateFromDateToken(requestBody, epicorHeaders = None):
   """  
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.PROC.PositivePaySvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
class ChangeBankAcctID_input:
   ...
   def __init__(self, obj):
      ...
      self.ds:list[Erp_Tablesets_PositivePayTableset] = obj["ds"]
      pass
...
class Erp_Tablesets_PositivePayParamRow:
   def __init__(self, obj):
      self.BankAcctID:str = obj["BankAcctID"]
      ...
      self.OutputFileName:str = obj["OutputFileName"]
      """  Output Filename  """  
      ...
      self.FromCheckNum:int = obj["FromCheckNum"]
      ...
      self.ToCheckNum:int = obj["ToCheckNum"]
      """  Check number is assigned during the check printing process for checks that are printed by the system or entered by the user for hand written checks.  """  
...
class Erp_Tablesets_PositivePayTableset:
   def __init__(self, obj):
      self.PositivePayParam:list[Erp_Tablesets_PositivePayParamRow] = obj["PositivePayParam"]
      self.ExtensionTables:list[Ice_Extensions_ExtensionTableData] = obj["ExtensionTables"]
      pass

class GetActualDateFromDateToken_input:
   """ Required : 
   date
   dateToken
   """  
   def __init__(self, obj):
      ...
      self.dateToken:str = obj["dateToken"]
      """  Date Token Value  """  
      pass

class GetActualDateFromDateToken_output:
   def __init__(self, obj):
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_PositivePayTableset] = obj["returnObj"]
      pass
