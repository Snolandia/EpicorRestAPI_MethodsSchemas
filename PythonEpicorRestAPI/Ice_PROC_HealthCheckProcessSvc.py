...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Ice.PROC.HealthCheckProcessSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Ice.PROC.HealthCheckProcessSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Ice.PROC.HealthCheckProcessSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_SubmitToAgent(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Ice.PROC.HealthCheckProcessSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
class GetDefaults_input:
   ...
   def __init__(self, obj):
      self.ds:list[Ice_Tablesets_HealthCheckProcessTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Ice_Tablesets_HealthCheckProcessTableset] = obj["returnObj"]
      pass
...
class Ice_Tablesets_HealthCheckProcessParamRow:
   def __init__(self, obj):
      self.SysRowID:str = obj["SysRowID"]
...
class Ice_Tablesets_HealthCheckProcessTableset:
   def __init__(self, obj):
      self.HealthCheckProcessParam:list[Ice_Tablesets_HealthCheckProcessParamRow] = obj["HealthCheckProcessParam"]
