...
async def getServiceDocument(epicorHeaders=None):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            configEpicorSchemas.epicorURL + "Ice.RPT.HealthCheckReportSvc/",
            headers=creds,
        ) as resp:
...
async def get_metadata(epicorHeaders=None):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            configEpicorSchemas.epicorURL + "Ice.RPT.HealthCheckReportSvc/$metadata",
            headers=creds,
        ) as resp:
            return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders=None):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            configEpicorSchemas.epicorURL
            + "Ice.RPT.HealthCheckReportSvc/$metadata"
            + params,
            headers=creds,
        ) as resp:
            return await resp.json()
...
async def get_GetParamsFromAgent(
    agentID, agentSchedNum, agentTaskNum, epicorHeaders=None
):
    """
...
class GetDefaults_input:
    ...
    def __init__(self, obj):
        self.ds: list[Ice_Tablesets_HealthCheckReportTableset] = obj["ds"]
        pass
...
class GetNewParameters_output:
    def __init__(self, obj):
        self.returnObj: list[Ice_Tablesets_HealthCheckReportTableset] = obj["returnObj"]
        pass
...
class Ice_Tablesets_HealthCheckReportParamRow:
    def __init__(self, obj):
        self.SysRowID: str = obj["SysRowID"]
...
class Ice_Tablesets_HealthCheckReportTableset:
    def __init__(self, obj):
        self.HealthCheckReportParam: list[Ice_Tablesets_HealthCheckReportParamRow] = (
            obj["HealthCheckReportParam"]
        )
        self.ReportStyle: list[Ice_Tablesets_ReportStyleRow] = obj["ReportStyle"]
        self.ExtensionTables: list[Ice_Extensions_ExtensionTableData] = obj[
            "ExtensionTables"
        ]
        pass
...
mfgsysdata\EDI  """
        ...
        self.StructuredOutputCertificateIsAllComp: bool = obj[
            "StructuredOutputCertificateIsAllComp"
        ]
        self.StructuredOutputCertificateIsSystem: bool = obj[
            "StructuredOutputCertificateIsSystem"
        ]
        self.StructuredOutputCertificateExpirationDate: str = obj[
            "StructuredOutputCertificateExpirationDate"
        ]
