...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Ice.PROC.FileManagerSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Ice.PROC.FileManagerSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Ice.PROC.FileManagerSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_SubmitToAgent(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Ice.PROC.FileManagerSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
class GetDefaults_input:
   ...
   def __init__(self, obj):
      self.ds:list[Ice_Tablesets_FileManagerTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Ice_Tablesets_FileManagerTableset] = obj["returnObj"]
      pass
...
class Ice_Tablesets_FileManagerRow:
   def __init__(self, obj):
      self.Task:str = obj["Task"]
      self.LogDir:str = obj["LogDir"]
      self.TempDir:str = obj["TempDir"]
      self.ReportDir:str = obj["ReportDir"]
      self.ProcessDir:str = obj["ProcessDir"]
      self.ServerLogDir:str = obj["ServerLogDir"]
      self.ServerTempDir:str = obj["ServerTempDir"]
      self.SysRowID:str = obj["SysRowID"]
...
class Ice_Tablesets_FileManagerTableset:
   def __init__(self, obj):
      self.FileManager:list[Ice_Tablesets_FileManagerRow] = obj["FileManager"]
