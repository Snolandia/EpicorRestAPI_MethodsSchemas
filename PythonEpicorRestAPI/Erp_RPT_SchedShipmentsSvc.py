...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.SchedShipmentsSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.SchedShipmentsSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
Notes: This is to give the UI a visual on the values for the SortByOption.
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.RPT.SchedShipmentsSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.SchedShipmentsSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_SubmitToAgent(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.RPT.SchedShipmentsSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
class Erp_Tablesets_SchedShipmentsParamRow:
   def __init__(self, obj):
      ...
      """  Sort By Option  """  
      self.PlantShip:bool = obj["PlantShip"]
...
class Erp_Tablesets_SchedShipmentsTableset:
   def __init__(self, obj):
      self.SchedShipmentsParam:list[Erp_Tablesets_SchedShipmentsParamRow] = obj["SchedShipmentsParam"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_SchedShipmentsTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_SchedShipmentsTableset] = obj["returnObj"]
      pass
