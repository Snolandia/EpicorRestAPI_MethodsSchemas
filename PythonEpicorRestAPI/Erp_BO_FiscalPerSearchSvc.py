...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.FiscalPerSearchSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.FiscalPerSearchSvc/$metadata",headers=creds) as resp:
           return await resp.json()

async def get_FiscalPerSearches(select = None, filter = None, orderby = None, top = None, skip = None, inlinecount = None, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.FiscalPerSearchSvc/FiscalPerSearches",headers=creds) as resp:
           return await resp.json()

async def post_FiscalPerSearches(requestBody, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.FiscalPerSearchSvc/FiscalPerSearches", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def get_FiscalPerSearches_Company_FiscalCalendarID_FiscalYear_FiscalYearSuffix_FiscalPeriod(Company, FiscalCalendarID, FiscalYear, FiscalYearSuffix, FiscalPeriod, select = None, filter = None, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.FiscalPerSearchSvc/FiscalPerSearches(" + Company + "," + FiscalCalendarID + "," + FiscalYear + "," + FiscalYearSuffix + "," + FiscalPeriod + ")",headers=creds) as resp:
           return await resp.json()

async def patch_FiscalPerSearches_Company_FiscalCalendarID_FiscalYear_FiscalYearSuffix_FiscalPeriod(Company, FiscalCalendarID, FiscalYear, FiscalYearSuffix, FiscalPeriod, requestBody, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.patch(configEpicorSchemas.epicorURL + "Erp.BO.FiscalPerSearchSvc/FiscalPerSearches(" + Company + "," + FiscalCalendarID + "," + FiscalYear + "," + FiscalYearSuffix + "," + FiscalPeriod + ")", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def delete_FiscalPerSearches_Company_FiscalCalendarID_FiscalYear_FiscalYearSuffix_FiscalPeriod(Company, FiscalCalendarID, FiscalYear, FiscalYearSuffix, FiscalPeriod, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.delete(configEpicorSchemas.epicorURL + "Erp.BO.FiscalPerSearchSvc/FiscalPerSearches(" + Company + "," + FiscalCalendarID + "," + FiscalYear + "," + FiscalYearSuffix + "," + FiscalPeriod + ")",headers=creds) as resp:
           return await resp.json()
...
async def get_List(select = None, filter = None, orderby = None, top = None, skip = None, inlinecount = None, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.FiscalPerSearchSvc/List",headers=creds) as resp:
           return await resp.json()
...
async def get_GetRows(whereClauseFiscalPer, pageSize, absolutePage, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.FiscalPerSearchSvc/List" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_GetNewFiscalPer(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.FiscalPerSearchSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
class Erp_Tablesets_FiscalPerSearchTableset:
   def __init__(self, obj):
      self.FiscalPer:list[Erp_Tablesets_FiscalPerRow] = obj["FiscalPer"]
      self.ExtensionTables:list[Ice_Extensions_ExtensionTableData] = obj["ExtensionTables"]
      pass

class Erp_Tablesets_UpdExtFiscalPerSearchTableset:
   def __init__(self, obj):
      self.FiscalPer:list[Erp_Tablesets_FiscalPerRow] = obj["FiscalPer"]
      self.ExtensionTables:list[Ice_Extensions_ExtensionTableData] = obj["ExtensionTables"]
      pass
...
class GetByID_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_FiscalPerSearchTableset] = obj["returnObj"]
      pass
...
class GetNewFiscalPer_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_FiscalPerSearchTableset] = obj["ds"]
      self.fiscalCalendarID:str = obj["fiscalCalendarID"]
      self.fiscalYear:int = obj["fiscalYear"]
      self.fiscalYearSuffix:str = obj["fiscalYearSuffix"]
      pass
...
class UpdateExt_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_UpdExtFiscalPerSearchTableset] = obj["ds"]
