...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/$metadata",headers=creds) as resp:
           return await resp.json()

async def get_Waves(select = None, expand = None, filter = None, orderby = None, top = None, skip = None, inlinecount = None, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/Waves",headers=creds) as resp:
           return await resp.json()

async def post_Waves(requestBody, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/Waves", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def get_Waves_Company_WaveNum(Company, WaveNum, select = None, expand = None, filter = None, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/Waves(" + Company + "," + WaveNum + ")",headers=creds) as resp:
           return await resp.json()

async def patch_Waves_Company_WaveNum(Company, WaveNum, requestBody, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.patch(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/Waves(" + Company + "," + WaveNum + ")", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def delete_Waves_Company_WaveNum(Company, WaveNum, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.delete(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/Waves(" + Company + "," + WaveNum + ")",headers=creds) as resp:
           return await resp.json()

async def get_Waves_Company_WaveNum_WaveOrders(Company, WaveNum, select = None, filter = None, orderby = None, top = None, skip = None, inlinecount = None, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/Waves(" + Company + "," + WaveNum + ")/WaveOrders",headers=creds) as resp:
           return await resp.json()

async def get_Waves_Company_WaveNum_WaveOrders_Company_WaveNum_OrderNum_OrderLine_OrderRelNum_JobNum_AssemblySeq_MtlSeq_TFOrdNum_TFOrdLine_WaveOrderSeq(Company, WaveNum, OrderNum, OrderLine, OrderRelNum, JobNum, AssemblySeq, MtlSeq, TFOrdNum, TFOrdLine, WaveOrderSeq, select = None, filter = None, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/Waves(" + Company + "," + WaveNum + ")/WaveOrders(" + Company + "," + WaveNum + "," + OrderNum + "," + OrderLine + "," + OrderRelNum + "," + JobNum + "," + AssemblySeq + "," + MtlSeq + "," + TFOrdNum + "," + TFOrdLine + "," + WaveOrderSeq + ")",headers=creds) as resp:
           return await resp.json()

async def get_WaveOrders(select = None, filter = None, orderby = None, top = None, skip = None, inlinecount = None, epicorHeaders = None):
   """  
   Summary: Calls GetRows for the service
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/WaveOrders",headers=creds) as resp:
           return await resp.json()

async def post_WaveOrders(requestBody, epicorHeaders = None):
   """  
   Summary: Calls UpdateExt to create new item for the service
   Description: Calls UpdateExt to create new item for the service. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li></ul></div>
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/WaveOrders", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def get_WaveOrders_Company_WaveNum_OrderNum_OrderLine_OrderRelNum_JobNum_AssemblySeq_MtlSeq_TFOrdNum_TFOrdLine_WaveOrderSeq(Company, WaveNum, OrderNum, OrderLine, OrderRelNum, JobNum, AssemblySeq, MtlSeq, TFOrdNum, TFOrdLine, WaveOrderSeq, select = None, filter = None, epicorHeaders = None):
   """  
   Summary: Calls GetByID to retrieve the WaveOrder item
   Description: Calls GetByID to retrieve the WaveOrder item by specified keys. <div>OData-specific rules:<ul><li>OData $-parameters data are case-sensitive</li><li>String parameters should be specified in single quotes</li></ul></div>
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/WaveOrders(" + Company + "," + WaveNum + "," + OrderNum + "," + OrderLine + "," + OrderRelNum + "," + JobNum + "," + AssemblySeq + "," + MtlSeq + "," + TFOrdNum + "," + TFOrdLine + "," + WaveOrderSeq + ")",headers=creds) as resp:
           return await resp.json()

async def patch_WaveOrders_Company_WaveNum_OrderNum_OrderLine_OrderRelNum_JobNum_AssemblySeq_MtlSeq_TFOrdNum_TFOrdLine_WaveOrderSeq(Company, WaveNum, OrderNum, OrderLine, OrderRelNum, JobNum, AssemblySeq, MtlSeq, TFOrdNum, TFOrdLine, WaveOrderSeq, requestBody, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.patch(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/WaveOrders(" + Company + "," + WaveNum + "," + OrderNum + "," + OrderLine + "," + OrderRelNum + "," + JobNum + "," + AssemblySeq + "," + MtlSeq + "," + TFOrdNum + "," + TFOrdLine + "," + WaveOrderSeq + ")", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def delete_WaveOrders_Company_WaveNum_OrderNum_OrderLine_OrderRelNum_JobNum_AssemblySeq_MtlSeq_TFOrdNum_TFOrdLine_WaveOrderSeq(Company, WaveNum, OrderNum, OrderLine, OrderRelNum, JobNum, AssemblySeq, MtlSeq, TFOrdNum, TFOrdLine, WaveOrderSeq, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.delete(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/WaveOrders(" + Company + "," + WaveNum + "," + OrderNum + "," + OrderLine + "," + OrderRelNum + "," + JobNum + "," + AssemblySeq + "," + MtlSeq + "," + TFOrdNum + "," + TFOrdLine + "," + WaveOrderSeq + ")",headers=creds) as resp:
           return await resp.json()
...
async def get_List(select = None, filter = None, orderby = None, top = None, skip = None, inlinecount = None, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/List",headers=creds) as resp:
           return await resp.json()
...
# Custom methods:
#########################################################################
async def get_GetRows(whereClauseWave, whereClauseWaveOrder, pageSize, absolutePage, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/List" + params,headers=creds) as resp:
           return await resp.json()

async def get_GetByID(waveNum, epicorHeaders = None):
   """  
...
async def post_GetNewWave(requestBody, epicorHeaders = None):
   """  
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.BO.WaveSvc/List", json=requestBody,headers=creds) as resp:
           return await resp.json()

async def post_GetNewWaveOrder(requestBody, epicorHeaders = None):
   """  
...
# OData Schemas:
#########################################################################
class Epicor_RESTApi_Help_ODataSetResponse_System_Collections_Generic_List_Erp_Tablesets_WaveListRow:
   def __init__(self, obj):
      self.odatametadata:str = obj["odatametadata"]
      self.value:list[Erp_Tablesets_WaveListRow] = obj["value"]
      pass

class Epicor_RESTApi_Help_ODataSetResponse_System_Collections_Generic_List_Erp_Tablesets_WaveOrderRow:
   def __init__(self, obj):
      self.odatametadata:str = obj["odatametadata"]
      self.value:list[Erp_Tablesets_WaveOrderRow] = obj["value"]
      pass

class Epicor_RESTApi_Help_ODataSetResponse_System_Collections_Generic_List_Erp_Tablesets_WaveRow:
   def __init__(self, obj):
      self.odatametadata:str = obj["odatametadata"]
      self.value:list[Erp_Tablesets_WaveRow] = obj["value"]
      pass

class Erp_Tablesets_WaveListRow:
   def __init__(self, obj):
      ...
      """  Wave Description.  """  
      self.PickComplete:bool = obj["PickComplete"]
      """  The entire wave has been picked.  It can no longer be modified or used.  """  
      self.PickStarted:bool = obj["PickStarted"]
...
class Erp_Tablesets_WaveOrderRow:
   def __init__(self, obj):
      self.Company:str = obj["Company"]
      """  Company Identifier.  """  
      self.WaveNum:int = obj["WaveNum"]
      ...
      self.WaveOrderSeq:int = obj["WaveOrderSeq"]
      """  Wave Order Sequence  """  
      self.BulkPickComplete:bool = obj["BulkPickComplete"]
      ...
      self.WaveQty:int = obj["WaveQty"]
      """  Wave Quantity  """  
      self.WaveUM:str = obj["WaveUM"]
      """  Unit of measure.  """  
...
class Erp_Tablesets_UpdExtWaveTableset:
   def __init__(self, obj):
      self.Wave:list[Erp_Tablesets_WaveRow] = obj["Wave"]
      self.WaveOrder:list[Erp_Tablesets_WaveOrderRow] = obj["WaveOrder"]
...
class Erp_Tablesets_WaveListTableset:
   def __init__(self, obj):
      self.WaveList:list[Erp_Tablesets_WaveListRow] = obj["WaveList"]
      self.ExtensionTables:list[Ice_Extensions_ExtensionTableData] = obj["ExtensionTables"]
      pass
...
class Erp_Tablesets_WaveTableset:
   def __init__(self, obj):
      self.Wave:list[Erp_Tablesets_WaveRow] = obj["Wave"]
      self.WaveOrder:list[Erp_Tablesets_WaveOrderRow] = obj["WaveOrder"]
      self.ExtensionTables:list[Ice_Extensions_ExtensionTableData] = obj["ExtensionTables"]
...
class GetByID_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_WaveTableset] = obj["returnObj"]
      pass
...
class GetList_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_WaveListTableset] = obj["returnObj"]
      pass
...
class GetNewWaveOrder_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_WaveTableset] = obj["ds"]
      self.waveNum:int = obj["waveNum"]
      self.orderNum:int = obj["orderNum"]
      self.orderLine:int = obj["orderLine"]
      self.orderRelNum:int = obj["orderRelNum"]
...
class GetRows_input:
   ...
   """  
   def __init__(self, obj):
      self.whereClauseWave:str = obj["whereClauseWave"]
...
class UpdateExt_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_UpdExtWaveTableset] = obj["ds"]
