...
async def getServiceDocument(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.w2formSvc/",headers=creds) as resp:
...
async def get_metadata(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.w2formSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def get_GetTokenList(tokenDataType, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.get(configEpicorSchemas.epicorURL + "Erp.RPT.w2formSvc/$metadata" + params,headers=creds) as resp:
           return await resp.json()
...
async def post_GetRptArchiveList(epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.RPT.w2formSvc/$metadata",headers=creds) as resp:
           return await resp.json()
...
async def post_SubmitToAgent(requestBody, epicorHeaders = None):
   ...
   async with aiohttp.ClientSession() as session:
       async with session.post(configEpicorSchemas.epicorURL + "Erp.RPT.w2formSvc/$metadata", json=requestBody,headers=creds) as resp:
           return await resp.json()
...
class Erp_Tablesets_w2formTableset:
   def __init__(self, obj):
      self.w2param:list[Erp_Tablesets_w2paramRow] = obj["w2param"]
      self.ReportStyle:list[Erp_Tablesets_ReportStyleRow] = obj["ReportStyle"]
      self.ExtensionTables:list[Ice_Extensions_ExtensionTableData] = obj["ExtensionTables"]
      pass

class Erp_Tablesets_w2paramRow:
   def __init__(self, obj):
      self.TaxYear:int = obj["TaxYear"]
      self.NumSets:int = obj["NumSets"]
...
class GetDefaults_input:
   ...
   """  
   def __init__(self, obj):
      self.ds:list[Erp_Tablesets_w2formTableset] = obj["ds"]
      pass
...
class GetNewParameters_output:
   def __init__(self, obj):
      self.returnObj:list[Erp_Tablesets_w2formTableset] = obj["returnObj"]
      pass
